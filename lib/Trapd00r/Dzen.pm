...
sub np {
  ...
  $current{artist} = len_mod($mpd->current->artist, 40, '...'),
  $current{album}  = len_mod($mpd->current->album, 35, '...'),
  $current{title}  = len_mod($mpd->current->title, 40, '...'),
  $current{year}   = $mpd->current->date // 1900,

  $current{album} =~ s/å/a/g;
